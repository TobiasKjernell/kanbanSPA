    ...
    return (
        <div className="gantt-root" onClick={() => setColorPicker(null)}>
            <div className="gantt-topbar">
                ...
                <button className='psp-text-gold cursor-pointer border rounded-md px-2 text-sm hover:bg-[#cea86f] hover:text-zinc-900' onClick={() => todayRef.current?.scrollIntoView({behavior:'smooth', block:'end'})}>Scroll to today</button>   
                    ...
                    <div className="gantt-timeline-inner" style={{ width: totalWidth }}>  
                        ...
                        <div className="gantt-days" style={{ top: 40, height: 28 }}>
                            {Array.from({ length: totalDays }).map((_, i) => (
                                <div key={i} ref={i === TODAY_DAY ? todayRef : null} className={['gantt-day-cell show', i === TODAY_DAY ? 'today' : ''].join(' ')} style={{ width: DAY_WIDTH }}>
                                    {dayToDate(i).getDate()}
                                </div>
                            ))} 
                        </div>
